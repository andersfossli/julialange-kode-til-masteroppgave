...
println("="^80)
println("CORRECTED SHAPLEY SENSITIVITY TEST")
println("="^80)
println("\nThis test verifies the CORRECTED Shapley sensitivity implementation")
println("using replicated nested sampling for correlated inputs.")
println("\nMATHEMATICAL FIX:")
println("  OLD (WRONG): Computed residual variance Var(Y | X_S = one value)")
println("  NEW (CORRECT): Computes V(S) = Var(E[Y | X_S]) via nested sampling")
...
println("  - Total cost: ~96,000 model evaluations")
println("\nExpected runtime: ~10-15 minutes (depends on system)")
println("="^80)
